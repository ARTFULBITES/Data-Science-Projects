...
SELECT COUNT(facid) AS charge_free_members
FROM Facilities
...
SELECT DISTINCT CONCAT(m.surname,' ', m.firstname) AS member, f.name AS court_name
FROM Members AS m
...
SELECT sub.member, sub.name, sub.cost
FROM (
    SELECT DISTINCT CONCAT(M.surname, ' ', M.firstname) AS member, F.name,
    CASE 
